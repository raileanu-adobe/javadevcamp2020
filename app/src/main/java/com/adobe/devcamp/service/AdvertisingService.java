...
public class AdvertisingService<T> {
    ...
    public Map<Integer, T> selectAll(Class<T> clazz){
        final Map<Integer, String> usersAsString = advertisingDao.selectAll(clazz);
        final Map<Integer, T> all = new HashMap<>();

        System.out.println(usersAsString.entrySet());
        for(Map.Entry<Integer, String> entry: usersAsString.entrySet()){
            try {
                System.out.println(entry.getValue() + " "  + clazz);
...
//                System.out.println(all);
    ...
    public T selectById(Class<T> clazz, int id){
        final String userAsString = advertisingDao.selectById(clazz, id);

        try {
            return objectMapper.readValue(userAsString, clazz);
        } catch (JsonProcessingException e) {
            logger.error("Object {} couldn't be deserialized", userAsString);
        }
